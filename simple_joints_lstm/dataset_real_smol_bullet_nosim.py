...
class DatasetRealSmolBulletNoSim(Dataset):
    ...
    def format_data(self, idx):
        return (
            np.hstack((self.curr_real[idx], self.action[idx])),
            self.next_real[idx] - self.curr_real[idx],
            self.epi[idx]
        )
...
if __name__ == '__main__':
    dsr = DatasetRealSmolBulletNoSim(train=False)
    print("len test", len(dsr))

    print(dsr[10])

    dsr = DatasetRealSmolBulletNoSim(train=True)
    print("len train", len(dsr))

    print(dsr[10])

    for i in range(10,20):
        print("real t1:", dsr[i]["x"][:12].round(2))
        print("action_:", dsr[i]["x"][12:].round(2))
        print("real t2:", (dsr[i]["x"][:12] + dsr[i]["y"]).round(2))
        print("delta__:", (dsr[i]["y"]).round(2))
        print("===")

...
from .params_cheetah import *


class LstmSimpleNet2Cheetah(nn.Module):
    def __init__(self):
        super(LstmSimpleNet2Cheetah, self).__init__()

        # because the LSTM is looking at 1 element at a time, and each element has 4 values
        self.linear1 = nn.Linear(len(RELEVANT_COLUMNS), HIDDEN_NODES)
        self.lstm1 = nn.LSTM(HIDDEN_NODES, HIDDEN_NODES, LSTM_LAYERS)
        self.linear2 = nn.Linear(HIDDEN_NODES, len(RELEVANT_COLUMNS))

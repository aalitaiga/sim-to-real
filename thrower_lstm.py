...
MODEL_PATH = "./trained_models/thrower_l{}_h{}_d{}_wdrop{}_trained_{}.pt".format(
    LSTM_LAYERS,
    HIDDEN_NODES,
    args.dropout,
    args.wdrop,
...
MODEL_PATH_BEST = "./trained_models/thrower_l{}_h{}_d{}_wdrop{}_trained_{}_best.pt".format(
    LSTM_LAYERS,
    HIDDEN_NODES,
    args.dropout,
    args.wdrop,
...
net = LstmThrower(53, 14, use_cuda=CUDA, batch=batch_size,
...
if not os.path.isfile('normalization/thrower_{}/mean_obs.npy'.format(max_steps)):
    ...
    np.save('normalization/thrower_{}/mean_correction.npy'.format(max_steps), (data["r_transition_obs"] - data["s_transition_obs"])[:,:,:14].mean(axis=(0,1)))
    ...
    np.save('normalization/thrower_{}/std_correction.npy'.format(max_steps), (data["r_transition_obs"] - data["s_transition_obs"])[:,:,:14].std(axis=(0,1)))
    os._exit(0)
else:
    means = {
        ...
        'c': np.load('normalization/thrower_{}/mean_correction.npy'.format(max_steps)),
    }
    ...
    std = {
        ...
        'c': np.load('normalization/thrower_{}/std_correction.npy'.format(max_steps)),
    }

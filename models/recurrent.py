...
from blocks.bricks import application, Initializable, Linear
...
class RecurrentStack(BaseRecurrent, Initializable):
    ...
    def __init__(self, transitions, fork_prototype=None, states_name="states",
        ...
        self.skip_level = len(transitions) // 2
    ...
    def do_apply(self, *args, **kwargs):
        ...
        for level, transition in enumerate(self.transitions):
            ...
            # add the previous states of the transition
            ...
            if level > 0:
                ...
                inputs = self.forks[level - 1].apply(last_states[-1], as_list=True)

                if level > self.skip_level and self.skip_connections:
                    # add the input of symmetric layer for unet
                    inputs.extend(self.forks[2 * self.skip_level - level].apply(last_states[2 * self.skip_level - level], as_list=True))
            ...
            last_states.append(result[state_index])
